...
Jev chooses a **final placement** for each piece. The local planner searches legal paths using left, right, clockwise rotation with wall kicks, soft drop, and optional hold. Each TypeSafe criterion contains whether hold is used, the landing piece and cells, the resulting board after normal row removal, and board-shape measurements: holes, column heights, maximum height, aggregate height, and bumpiness. Rows cleared, score gained, and game-over status are not sent to Jev; it must infer those consequences from the board and rules. The shared context contains the current game state and ghost, without duplicating the candidate placements. Jev selects a placement ID through the TypeSafe Choice API; every option's probability is retained.
...
Equivalent landings are deduplicated, with hold and non-hold options kept distinct. Search keeps cells at or below y = -4, matching the model-state boundary. If more than 255 placements are reachable, the API limit is met by an internal shortlist; its derived measurements are not included in the TypeSafe request. Otherwise every reachable placement is offered. Resulting boards cover the current placement and normal row removal; they do not predict a full future game.
